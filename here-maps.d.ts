declare namespace H {
	...
	class Map extends H.util.EventTarget {
		 ...
		 */
		capture(
			callback: (a: HTMLCanvasElement | null) => any,
		...
		getObjectAt(
			x: number,
			y: number,
			callback: (a: H.map.Feature | undefined) => any
		): void;
		/**
		...
		getObjectsAt(
			x: number,
			y: number,
			callback: (a: H.map.Feature[]) => any,
			opt_allowDuplicates?: boolean
		): void;
		/**
		...
		getObjectsWithin(
			area: H.geo.Polygon,
			callback: (a: H.map.Object[]) => any,
			opt_options?: H.Map.GetObjectsWithinOptions
		): void;
		/**
		 ...
		 */
		storeContent(
			opt_onprogress?: (a: H.util.Request) => any,
			opt_boundingBox?: H.geo.Rect,
			opt_min?: number,
			opt_max?: number,
...
declare namespace H.Map {
	...
	type Options = {
		...
		imprint?: H.map.Imprint.Options | null;
...
declare namespace H.clustering.Provider {
	...
	type Options = {
		...
		getCopyrights?: (a: H.geo.Rect, b: number) => any;
		max?: number;
		min?: number;
		pixelRatio?: number;
...
declare namespace H.data.heatmap {
	...
	class Colors {
		 ...
		 */
		constructor(
			stops: { [/* warning: coerced from ? */ key: string]: string },
...
declare namespace H.data.heatmap {
	...
	class Provider extends H.map.provider.RemoteTileProvider {
		 ...
		 */
		addData(
			dataPoints: H.data.heatmap.IDataPoint[],
			opt_hardReload?: boolean
		): void;
		/**
...
declare namespace H.geo {
	...
	class LineString extends H.geo.AbstractGeometry {
		 ...
		 */
		eachLatLngAlt(
			eachFn: (a: number, b: number, c: number, d: number) => any,
			opt_start?: number,
			opt_end?: number
		): void;
...
declare namespace H.map {
	...
	class ChangeEvent extends H.util.ChangeEvent {
		...
		constructor(
			type: string,
			newValue: GlobalObject,
			oldValue: GlobalObject,
			modifiers: number
		);
		/**
...
declare namespace H.map.DomIcon {
	type Options = {
		onAttach?: (a: GlobalElement, b: H.map.DomIcon, c: H.map.DomMarker) => any;
		onDetach?: (a: GlobalElement, b: H.map.DomIcon, c: H.map.DomMarker) => any;
	};
}
declare namespace H.map {
	/**
...
declare namespace H.map {
	...
	class Group extends H.map.Object {
		 ...
		 */
		forEach(
			callback: (a: H.map.Object, b: number, c: H.map.Group) => any,
...
declare namespace H.map {
	...
	interface IControl {
		...
		endControl(
			...
			opt_adjustView?: (a: H.map.ViewModel.ILookAtData) => any
		): void;
		/**
...
declare namespace H.map {
	...
	class Imprint extends H.util.Disposable implements H.util.ICapturable {
		...
		capture(
			...
			pixelRatio: number,
			callback: (a: HTMLCanvasElement) => any,
			opt_errback?: (a: string) => any
		): void;
...
declare namespace H.map.Rect {
	type Options = {
		...
		max?: number;
		min?: number;
		provider?: H.map.provider.ObjectProvider | null;
		style?: H.map.SpatialStyle.Options;
		visibility?: boolean;
		volatility?: boolean;
		zIndex?: number;
...
declare namespace H.map.SpatialStyle {
	...
	type Options = {
		...
		lineCap?: string;
		lineDash?: number[];
		lineDashImage?: HTMLImageElement | HTMLCanvasElement;
		lineDashOffset?: number;
		lineHeadCap?: string;
		lineJoin?: string;
		lineTailCap?: string;
...
declare namespace H.map.layer {
	...
	class CanvasLayer extends H.map.layer.Layer {
		...
		constructor(
			renderCallback: (
				a: CanvasRenderingContext2D | WebGLRenderingContext,
				b: H.map.render.RenderingParams
			) => H.map.render.RenderState,
			opt_options?: H.map.layer.CanvasLayer.Options
		);
	}
}
declare namespace H.map.layer.CanvasLayer {
	type Options = {
		contextAttributes?: GlobalObject;
		contextType?: string;
		dark?: boolean;
		max?: number;
		min?: number;
...
declare namespace H.map.layer {
	...
	class DomLayer extends H.map.layer.Layer {
		...
		constructor(
			...
			opt_options?: H.map.layer.Layer.Options
		);
	}
}
declare namespace H.map.layer {
...
declare namespace H.map.layer {
	...
	class ObjectLayer
		...
		implements H.map.layer.ITileLayer, H.map.layer.IMarkerLayer
	{
		/**
		 * This class represents a layer which renders map objects. Spatial objects such
		 * as polygons and polylines are rendered to tiles before being passed to the
...
declare namespace H.map.provider.ImageTileProvider {
	type Options = {
		...
		engineType?: H.Map.EngineType;
		getCopyrights?: (a: H.geo.Rect, b: number) => any;
		getURL: (a: number, b: number, c: number) => string | undefined;
		headers?: GlobalObject;
		max?: number;
		min?: number;
		opacity?: number;
		style?: H.map.render.Style<any>;
		tileSize?: number;
		uri?: string;
	};
}
...
declare namespace H.map.provider {
	...
	class LocalObjectProvider extends H.map.provider.ObjectProvider {
		...
		getObjectsWithin(
			area: H.geo.Polygon,
			callback: (a: H.map.Object[]) => any,
			options: H.map.provider.LocalObjectProvider.GetObjectsWithinOptions
		): void;
		/**
		...
		setStyleInternal(
			style: H.map.render.Style<any>,
			opt_suppressChangeEvent?: boolean
		): void;
	}
}
...
declare namespace H.map.provider {
	...
	abstract class RemoteTileProvider extends H.map.provider.TileProvider {
		...
		createTileInternal(
			...
			y: number,
			z: number,
			data: HTMLImageElement | HTMLCanvasElement | GlobalObject,
			opt_options?: H.service.ServiceParameters
		): H.map.provider.Tile;
		/**
...
declare namespace H.map.render {
	...
	abstract class RenderEngine
		...
		implements H.util.animation.IAnimatable
	{
		/**
		 * This is an abstract class representing a render engine.
		 * Render engines are used to render the geographical position from a view model on the screen (viewport element).
...
declare namespace H.map.render.p2d.RenderEngine {
	type Options = {
		...
		pixelRatio: number;
		renderBaseBackground?: GlobalObject;
	};
}
declare namespace H.map.render.webgl {
	...
	type StyleConfig = {
		...
		layers?: GlobalObject;
		scene?: GlobalObject;
		sources?: GlobalObject;
		textures?: GlobalObject;
	};
}
declare namespace H.map.render.webgl {
	/**
	...
	class RenderEngine extends H.map.render.RenderEngine {
		 ...
		 */
		geoToMeters(geoPoint: H.geo.IPoint): {
			x: number;
			y: number;
			z: number | null;
		};
...
declare namespace H.map.render.webgl {
	...
	class Style extends H.map.render.Style<any> {
		...
		extractConfig(
			layerIds: string[],
			opt_preserving?: boolean,
			opt_excludeParentDraws?: boolean
		): H.map.render.webgl.StyleConfig;
		getBaseUrl(): string | undefined;
		...
		setProperty(
			path: string,
			value: string | number | GlobalObject | boolean,
			opt_force?: boolean
		): void;
		/**
...
declare namespace H.mapevents {
	...
	class Event extends H.util.Event {
		...
		constructor(
			...
			target: H.Map | null | H.map.Object,
			originalEvent: GlobalEvent
		);
		/**
...
declare namespace H.service.GeocodingService {
	type Options = {
		...
		headers?: H.service.ServiceParameters;
		path?: string;
...
declare namespace H.service.MapTileService {
	type Info = {
		formats: { [key: string]: GlobalObject };
		languages: { [key: string]: GlobalObject };
		maps: { [key: string]: GlobalObject };
		resolutions: { [key: string]: GlobalObject };
		schemes: { [key: string]: GlobalObject };
		tiletypes: { [key: string]: GlobalObject };
	};
	type Options = {
		baseUrl?: H.service.Url;
		headers?: GlobalObject;
...
declare namespace H.service {
	...
	class PlacesService extends H.service.Service {
		...
		follow(
			...
			onResult: (a: GlobalObject) => any,
			onError: (a: Error) => any,
			opt_additionalParameters?: GlobalObject
		): H.util.ICancelable;
		/**
...
declare namespace H.service {
	...
	class Platform {
		 ...
		 */
		createHybridLayers(
			styleConfig: GlobalObject | string,
			onSuccess: (a: { [key: string]: H.map.layer.TileLayer }) => any,
			onError: (a: Error) => any
		): void;
		/**
		 ...
		 */
		createService(
			Service: { new (a?: GlobalObject): GlobalObject },
			opt_options?: GlobalObject
		): H.service.Service;
		/**
		 ...
		 */
		getIMLService(
			opt_options?: H.service.iml.Service.Options
		 ...
		 */
		getMetaInfoService(
			opt_options?: H.service.metaInfo.Service.Options
		 ...
		 */
		getOMVService(
			opt_options?: H.service.omv.Service.Options
		...
		getRasterTileService(
			opt_options?: H.service.rasterTile.Service.Options
		...
		getRoutingService(
			opt_options?: H.service.Options,
			opt_version?: number
		): H.service.RoutingService | H.service.RoutingService8;
		/**
		...
		getTrafficService(
			...
			opt_version?: number
		): H.service.traffic.Service | H.service.traffic.Service7;
		/**
		...
		getTrafficVectorTileService(
			opt_options?: H.service.trafficVectorTile.Service.Options
...
declare namespace H.service.extension {
	 ...
	 */
	type RowFilter = (
		a: H.service.extension.dataView.IRow<any, any>,
		b: string
	) => boolean;
}
declare namespace H.service.extension {
...
declare namespace H.service.extension.TileProvider {
	type Options = {
		...
		styleCallback?: H.service.extension.TileProvider.StyleCallback;
		tileCacheSize?: number;
		tileSize?: number;
		uri?: string;
	};
	...
	type StyleCallback = (
		a: H.service.extension.dataView.IRow<any, any>
	) => H.map.Icon | H.map.DomIcon | H.map.SpatialStyle.Options;
}
declare namespace H.service.extension.customLocation {
	...
	type LayerConfig = {
		columns: string[] | null;
		layerId: string;
		rowFilter?: H.service.extension.RowFilter;
		rowFilterContext?: GlobalObject;
	};
}
declare namespace H.service.extension.customLocation {
	/**
	...
	type Grant = {
		grantedAppId: string;
		permissions: H.service.extension.customLocation.Permission[];
	};
}
declare namespace H.service.extension.customLocation {
	/**
	...
	type MetaInfo = {
		columnNames: string[];
		grants?: H.service.extension.customLocation.Grant[];
		lastUpdateTimestamp?: number;
		layerId: string;
		level?: number;
		storage?: H.service.extension.customLocation.Storage;
	};
}
declare namespace H.service.extension.customLocation {
	/**
...
declare namespace H.service.extension.customLocation {
	...
	class Service extends H.service.Service {
		 ...
		 */
		appendRows(
			rows: H.service.extension.customLocation.Table.Row[],
			onResult: (
				a: H.service.extension.customLocation.Table.Row[],
				b: boolean
			) => any,
			onError: (
				a: Error,
				b: H.service.extension.customLocation.Table.Row[],
				c: boolean
			) => any
		): H.util.ICancelable;
		/**
		...
		createLayer(
			...
			onResult: (a: H.service.extension.customLocation.Table) => any,
			onError: (a: Error) => any
		): H.util.ICancelable;
		/**
...
declare namespace H.service.extension.customLocation {
	...
	class Table implements H.service.extension.dataView.ITable<any, any> {
		...
		concat(
			...var_args: H.service.extension.dataView.ITable<any, any>[]
		): H.service.extension.dataView.ITable<any, any>;
		getCell(rowIndex: number, columnName: string): any | undefined;
		getColumn(
			columnName: string
		): H.service.extension.dataView.IColumn<any> | undefined;
		getColumnNames(): string[];
		getMeta(): any;
		getRow(
			rowIndex: number
		): H.service.extension.dataView.IRow<any, any> | undefined;
		getRowCount(): number;
		/**
...
declare namespace H.service.extension.customLocation {
	...
	class TileProvider extends H.service.extension.TileProvider {
		 ...
		 */
		constructor(
			service: H.service.extension.customLocation.Service,
			layerConfig: H.service.extension.customLocation.LayerConfig,
			options: H.service.extension.TileProvider.Options
		);
		requestInternal(
...
declare namespace H.service.extension.dataView {
	...
	interface ITable<C = any, M = any> {
		...
		getRow(
			rowIndex: number
		): H.service.extension.dataView.IRow<C, any> | undefined;
		/**
...
declare namespace H.service.extension.platformData {
	 ...
	 */
	type Table = H.service.extension.dataView.ITable<
		string | (Uint16Array | GlobalFloat32Array)[] | null,
		H.service.ServiceParameters | undefined
	> | null;
}
declare namespace H.service.extension.platformData {
	/**
	...
	class TileProvider extends H.service.extension.TileProvider {
		 ...
		 */
		constructor(
			service: H.service.extension.platformData.Service,
			layerConfig: H.service.extension.platformData.LayerConfig,
			options: H.service.extension.TileProvider.Options
		);
		requestInternal(
...
declare namespace H.service.iml {
	...
	class Provider extends H.map.provider.RemoteTileProvider {
		 ...
		 */
		constructor(
			service: H.service.iml.Service,
			catalogHrn: string,
			layerId: string,
			opt_options?: H.service.iml.Provider.Options
		);
		/**
...
declare namespace H.service.iml {
	...
	class Service extends H.service.Service {
		...
		getFeaturesByTile(
			...
			catalogHrn: string,
			layerId: string,
			onResult: (a: ArrayBuffer | GlobalObject) => any,
			onError: (a: Error) => any,
			opt_params?: H.service.ServiceParameters,
			opt_geojsonFormat?: boolean
		): H.util.ICancelable;
		/**
...
declare namespace H.service.omv {
	...
	class Provider extends H.map.provider.RemoteTileProvider {
		...
		constructor(
			...
			style: H.map.render.Style<any>,
			opt_options?: H.service.omv.Provider.Options
		);
		getCache(): H.util.ICache;
		getCopyrights(
			boundingBox: H.geo.Rect,
...
declare namespace H.service.omv {
	...
	class Service extends H.service.Service {
		...
		createLayer(
			style: H.map.render.Style<any>,
			opt_providerOptions?: H.service.omv.Provider.Options,
			opt_layerOptions?: H.map.layer.ITileLayer.Options
		): H.map.layer.TileLayer;
		/**
...
declare namespace H.service.omv.Service {
	type Options = {
		...
		mapVersion?: string;
		path?: string;
		queryParams?: H.service.ServiceParameters;
		subDomain?: string;
	};
}
...
declare namespace H.service.rasterTile {
	...
	class Provider extends H.map.provider.ImageTileProvider {
		 ...
		 */
		constructor(
			service: H.service.rasterTile.Service,
			opt_options?: H.service.rasterTile.Provider.Options
		);
		getCopyrights(
			boundingBox: H.geo.Rect,
			level: number
...
declare namespace H.service.traffic {
	...
	class Service extends H.service.Service {
		...
		createFlowLayer(
			opt_additionalParameters?: H.service.ServiceParameters,
			opt_engineType?: H.Map.EngineType
		): H.map.layer.TileLayer;
		/**
...
declare namespace H.service.traffic.flow {
	...
	class Provider extends H.map.provider.RemoteTileProvider {
		 ...
		 */
		constructor(
			service: H.service.traffic.Service,
			opt_refreshTime?: number,
			opt_additionalParameters?: H.service.ServiceParameters,
			opt_engineType?: H.Map.EngineType
		);
		createTileInternal(
...
declare namespace H.service.trafficVectorTile {
	...
	class Provider extends H.map.provider.RemoteTileProvider {
		 ...
		 */
		constructor(
			service: H.service.trafficVectorTile.Service,
			style: H.map.render.Style<any>,
			opt_options?: H.service.trafficVectorTile.Provider.Options
		);
		getCache(): H.util.ICache;
		getCopyrights(
			boundingBox: H.geo.Rect,
...
declare namespace H.service.trafficVectorTile {
	...
	class Service extends H.service.Service {
		...
		createLayer(
			style: H.map.render.Style<any>,
			opt_providerOptions?: H.service.trafficVectorTile.Provider.Options,
			opt_layerOptions?: H.map.layer.ITileLayer.Options
		): H.map.layer.TileLayer;
		/**
		 * The property name to use when specifying options for this service
...
declare namespace H.ui.DistanceMeasurement {
	type Options = {
		alignment?: string;
		distanceFormatter?: (a: number) => string;
		endIcon?: H.map.Icon;
		lineStyle?: H.map.SpatialStyle.Options;
...
declare namespace H.ui.InfoBubble {
	type Options = {
		content?: string | Node;
		onStateChange?: (a: H.ui.InfoBubble.State) => any;
	};
	/**
...
declare namespace H.ui.MapSettingsControl {
	type Entry = {
		label: string;
		layer?: H.map.layer.Layer[] | H.map.layer.Layer;
	};
	type Options = {
		alignment?: string;
...
declare namespace H.ui {
	...
	class Overview extends H.ui.Control {
		...
		constructor(
			baseLayer: H.map.layer.Layer,
			opt_options?: H.ui.Overview.Options
		);
		renderInternal(element: GlobalElement, doc: Document): void;
		/**
...
declare namespace H.ui.UI {
	type Options = {
		...
		zoomrectangle?: H.ui.ScaleBar.Options | boolean;
	};
}
declare namespace H.ui {
	/**
...
declare namespace H.ui.base.Button {
	type Options = {
		...
		disabled?: boolean;
		label?: string;
		onStateChange?: (a: H.util.Event) => any;
	};
	/**
...
declare namespace H.ui.base {
	...
	class Container extends H.ui.base.Element {
		...
		constructor(
			...
			opt_children?: H.ui.base.Element[]
		);
		/**
...
declare namespace H.ui.base {
	...
	class OverlayPanel extends H.ui.base.Container {
		 ...
		 */
		setState(
			state: H.ui.base.OverlayPanel.State,
			opt_force?: boolean
...
declare namespace H.ui.base.RadioGroup {
	type Options = {
		buttonsOptions?: H.ui.base.Button.Options[];
		title?: string;
	};
}
...
declare namespace H.util {
	...
	class AsyncQueue {
		 ...
		 */
		constructor(
			mode: H.util.AsyncQueue.Mode,
			processFn: (a: H.util.AsyncQueue.Task, b: any) => any
		);
		/**
		...
		push(
			data: any,
			onDone: (a: H.util.AsyncQueue.Task, b: any, c: any) => any
...
declare namespace H.util {
	...
	class Cache implements H.util.ICache {
		...
		constructor(
			maxSize: number,
			opt_onDrop?: (a: string, b: any, c: number) => any,
			opt_filter?: (a: string, b: any, c: number) => boolean
		);
		/**
		...
		drop(id: any): void;
		forEach(
			callback: (a: string, b: any, c: number) => any,
			opt_ctx?: GlobalObject,
			opt_matcher?: (a: string, b: any, c: number) => boolean
		): void;
		/**
...
declare namespace H.map.render.harp {
	...
	class Style extends H.map.render.Style<any> {
		 ...
		 */
		setEnabledFeatures(
			enabledFeatures?: { feature: string; mode?: string }[]
		): void;
